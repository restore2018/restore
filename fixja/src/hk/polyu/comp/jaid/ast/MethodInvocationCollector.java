...
public class MethodInvocationCollector extends ASTVisitor {
    ...
    public MethodInvocationCollector(boolean shouldCheckType) {
        super(false);
    ...
    public Set<Expression> getSubExpressionSetWithinClass() {
        return subExpressionSetWithinClass;
    }

    // =============================== Storage

    private Set<Expression> subExpressionSet;
    private Set<Expression> subExpressionSetWithinClass;
    ...
    public boolean visit(MethodInvocation node) {
        subExpressionSet.add(node.getName());

        if (node.getExpression() != null)
            node.getExpression().accept(this);
        if (node.arguments() != null && !node.arguments().isEmpty()) {
            node.arguments().stream().filter(x->x instanceof MethodInvocation).forEach(x->subExpressionSet.add(((MethodInvocation)x).getName()));
